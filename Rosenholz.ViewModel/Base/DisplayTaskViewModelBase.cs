            ...
            get
            {
                if (_openAUFolderCommand == null)
                {
                    _openAUFolderCommand = new RelayCommand(
                        (parameter) => Process.Start(new ProcessStartInfo(Path.Combine(Rosenholz.Settings.Settings.Instance.StorageBaseLocation, "ZAV", Entry.AUReference.Substring(Entry.AUReference.Length - 2, 2), Entry.AUReference)) { UseShellExecute = true }),
